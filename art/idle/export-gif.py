"""Export the current idle atlas as native and 4x looping GIFs (requires Pillow)."""
import json
...
spec = json.loads((folder / 'idle-poses.json').read_text())
atlas = Image.open(folder.parent.parent / 'assets/spirit-idle.png').convert('RGBA')
...
for scale, name in [(1, 'idle.gif'), (4, 'idle-4x.gif')]:
    output = folder / name
